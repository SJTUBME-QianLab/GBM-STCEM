...
class ResSpiNet(nn.Module):
    ...
    def forward(self, input):
        ...
        feature = self.conv1(input_rand)
        feature = self.bn1(feature)
        feature = self.relu(feature)
        feature = self.maxpool(feature)
        out_l1 = self.layer1(feature)
        ...
        act_feat = out_l4[0:int(L / 2)] - out_l4[int(L / 2):L]
        act_feat = self.avgpool(act_feat)
        res_feat = self.avgpool(out_l4[0:int(L / 2)])
        feat = act_feat.reshape(act_feat.size(0), -1)
        res_feat = res_feat.reshape(res_feat.size(0), -1)
        feat = F.normalize(self.head(feat)+self.head(res_feat), dim=1)
        return feat, out_features
...
def resnext101_32x8d(pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnext101_32x8d', Bottleneck, [3, 4, 23, 3],
                   pretrained, progress, **kwargs)
model_dict = {
    'resnet18': [resnet18(pretrained=True), 512],
